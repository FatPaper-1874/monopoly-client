...
const canStart = computed(
    () =>
        !(Boolean(gameSetting.value.mapId) && roomInfoStore.userList.every((user) => user.userId === ownerId.value || user.isReady))
);

watch(gameSetting, async (newGameSetting, oldGameSetting) => {
...
<template>
    ...
    <div class="left-container">
      ...
      <div class="map-option">
        ...
        <div class="options">
          ...
          <div>
            <input :disabled="!isOwner" type="number" min="1000" step="1000" v-model="_tempGameSettingFrom.initMoney"/>￥
          </div>
        </div>
        <div class="options">
          <span class="label">倍率涨幅</span>
          <div>
            <input :disabled="!isOwner" type="number" min="0" v-model="_tempGameSettingFrom.multiplier"/>倍
          </div>
        </div>
        <div class="options">
          <span class="label">涨幅频率</span>
          <div>
            <input :disabled="!isOwner" type="number" min="1" v-model="_tempGameSettingFrom.multiplierIncreaseRounds"/>回合
          </div>
        </div>
  ...
  <FpDialog @submit="handleChangeMap" v-model:visible="mapSelectorVisible">
    ...
    <template #default>
      <ItemSelector :column="1" :multiple="false" :item-list="_mapList" key-name="id"
